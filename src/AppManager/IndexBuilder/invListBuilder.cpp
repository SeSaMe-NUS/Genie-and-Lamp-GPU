...
void invListBuilder::getSampleQuery(vector<int>& data, int queryNum , int queryLen, map<uint, vector<int> >& _query, bool rnd){
	...
	for (int i = 0; i < queryNum; i++) {
		...
		//for (int j = st; j < st + dim; j++) {
			//int d=(j<<keyLow);//move key to high bits
			//d+=data.at(st+j);//add the value at this dim
			int d = data.at(st+j);
...
void invListBuilder::runBuildPesudoGroupIdx(
	...
	//dp.writeInvListQueryFile(queryName.c_str(), *query, keyLow);
	dp.writeQueryFile(groupQueryName.c_str(), query);
 ...
 */
void invListBuilder::runBuildQuery(string fName, int fCol, string ft, string queryName,int queryNum, int dimIn,bool rand){
	...
	map <uint, vector <int> > query;
	getSampleQuery(data,queryNum,query, rand);
