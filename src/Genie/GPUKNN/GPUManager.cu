...
void GPUManager::conf_GPUManager_GPUSpecification(GPUSpecification& gpu_spec){
		...
		get_invert_list_from_binary_file( gpu_spec.invertedListPath, numofDim, maxValuePerDim,maxFeatureID);
...
void GPUManager::init_GPU_query( vector<GpuQuery>& query_set )
{
	...
	d_query_feature.clear();
	d_query_feature.resize((unsigned int) invert_list_spec_host.maxFeatureID * invert_list_spec_host.numOfQuery);
	d_query_feature.shrink_to_fit();
 ...
 */
void GPUManager::get_invert_list_from_binary_file ( string filename, int& _numberOfDim,  int& _maxValuePerDim, int& _maxFeatureID )
{
		...
		else
		{
			...
			if(dim>=invert_list_spec_host.totalDimension){
				cerr<<"error::dim is outside of maximum possible dimensions"<<endl;
				exit(1);
			}

			if(dim_value<invert_list_spec_host.indexDimensionEntry[dim].minDomain||dim_value>invert_list_spec_host.indexDimensionEntry[dim].maxDomain ){
				cerr<<"error:input invert list has problem for dimension values, should be between min and max dimension value"<<endl;
				exit(1);
			}
 ...
 */
void GPUManager::readQueryFromFile( string queryFileName, vector<GpuQuery>& _query_set )
{
...
void GPUManager::update_windowQuery_entryAndTable(QueryInfo* host_queryInfo, int d_query_info_id){
 ...
 *
 */
void GPUManager::update_QueryInfo_entry_fromHostToDevice(QueryInfo* host_queryInfo, QueryInfo* device_queryInfo){

		//copy data

		cudaMemcpy(&(device_queryInfo->topK), &(host_queryInfo->topK), sizeof(int), cudaMemcpyHostToDevice);
		...
		cudaMemcpy(&(device_queryInfo->aggregateFunc), &(host_queryInfo->aggregateFunc), sizeof(int), cudaMemcpyHostToDevice);

		int size = host_queryInfo->numOfDimensionToSearch;
		QueryInfo* d2h_queryInfo = (QueryInfo*) malloc(sizeof(QueryInfo));
		cudaMemcpy(d2h_queryInfo, device_queryInfo, sizeof(QueryInfo), cudaMemcpyDeviceToHost);

		cudaMemcpy(d2h_queryInfo->searchDim, host_queryInfo->searchDim, sizeof(int) * size, cudaMemcpyHostToDevice);
		cudaMemcpy(d2h_queryInfo->distanceFunc, host_queryInfo->distanceFunc, sizeof(float) * size, cudaMemcpyHostToDevice);
		cudaMemcpy(d2h_queryInfo->upperBoundSearch, host_queryInfo->upperBoundSearch, sizeof(int) * size, cudaMemcpyHostToDevice);
		cudaMemcpy(d2h_queryInfo->lowerBoundSearch, host_queryInfo->lowerBoundSearch, sizeof(int) * size, cudaMemcpyHostToDevice);
		cudaMemcpy(d2h_queryInfo->lastPos, host_queryInfo->lastPos, sizeof(int2) * size, cudaMemcpyHostToDevice);
		cudaMemcpy(d2h_queryInfo->keyword, host_queryInfo->keyword, sizeof(float) * size, cudaMemcpyHostToDevice);
		cudaMemcpy(d2h_queryInfo->dimWeight, host_queryInfo->dimWeight, sizeof(float) * size, cudaMemcpyHostToDevice);
...
void GPUManager::update_QueryInfo_upperAndLowerBound_fromHostToDevice(QueryInfo* device_queryInfo, vector<float>& new_upperBoundDist, vector<float>& new_lowerBoundDist){
...
void GPUManager::copy_QueryInfo_vec_fromHostToDevice(  host_vector<QueryInfo*>& hvec, device_vector<QueryInfo*>& _dvec )
{
	...
	for ( int i = 0; i < hvec.size(); i++ )
	{
		...
		cudaMemcpy(devQueryInfo, hostQueryInfo, sizeof(QueryInfo), cudaMemcpyHostToDevice);
		...
		cudaMemcpy(distanceFunc, hostQueryInfo->distanceFunc, sizeof(float) * size, cudaMemcpyHostToDevice);
		cudaMemcpy(upperBoundSearch, hostQueryInfo->upperBoundSearch, sizeof(int) * size, cudaMemcpyHostToDevice);
		cudaMemcpy(lowerBoundSearch, hostQueryInfo->lowerBoundSearch, sizeof(int) * size, cudaMemcpyHostToDevice);
		cudaMemcpy(lastPos, hostQueryInfo->lastPos, sizeof(int2) * size, cudaMemcpyHostToDevice);
		...
		// copy pointer to gpu
		cudaMemcpy(&(devQueryInfo->searchDim), &searchDim, sizeof(int *), cudaMemcpyHostToDevice);
		cudaMemcpy(&(devQueryInfo->distanceFunc), &distanceFunc, sizeof(float *), cudaMemcpyHostToDevice);
		cudaMemcpy(&(devQueryInfo->upperBoundSearch), &upperBoundSearch, sizeof(int *), cudaMemcpyHostToDevice);
		cudaMemcpy(&(devQueryInfo->lowerBoundSearch), &lowerBoundSearch, sizeof(int *), cudaMemcpyHostToDevice);
		cudaMemcpy(&(devQueryInfo->lastPos), &lastPos, sizeof(int2 *), cudaMemcpyHostToDevice);
	...
	for ( int i = 0; i < _dvec.size(); i++ )
		{
			...
			int *searchDim;
			cudaMemcpy(&searchDim,&(devQueryInfo->searchDim), sizeof(int *), cudaMemcpyDeviceToHost);
			cudaFree(searchDim);

			float *distanceFunc;
			cudaMemcpy(&distanceFunc,&(devQueryInfo->distanceFunc), sizeof(float *), cudaMemcpyDeviceToHost);
			cudaFree(distanceFunc);

			int *upperBoundSearch;
			cudaMemcpy(&upperBoundSearch,&(devQueryInfo->upperBoundSearch), sizeof(int *), cudaMemcpyDeviceToHost);
			cudaFree(upperBoundSearch);

			int *lowerBoundSearch;
			cudaMemcpy(&lowerBoundSearch,&(devQueryInfo->lowerBoundSearch), sizeof(int *), cudaMemcpyDeviceToHost);
			cudaFree(lowerBoundSearch);

			int2 *lastPos;
			cudaMemcpy(&lastPos,&(devQueryInfo->lastPos), sizeof(int2 *), cudaMemcpyDeviceToHost);
			cudaFree(lastPos);
			...
			float *dimWeight;
			cudaMemcpy(&dimWeight,&(devQueryInfo->dimWeight), sizeof(float *), cudaMemcpyDeviceToHost);
			cudaFree(dimWeight);
